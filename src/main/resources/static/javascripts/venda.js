Drinks.Venda = (function() {
	...
	function onValorFreteAlterado(evento) {
		this.valorFrete = Drinks.recuperarValor($(evento.target).val());
	}
	
	function onValorDescontoAlterado(evento) {
		this.valorDesconto = Drinks.recuperarValor($(evento.target).val());
	}
	
	function onValoresAlterados() {
		var valorTotal = numeral(this.valorTotalItens) + numeral(this.valorFrete) - numeral(this.valorDesconto);
		this.valorTotalBox.html(Drinks.formatarMoeda(valorTotal));
...
$(function() {
	...
	var autocomplete = new Drinks.Autocomplete();
	autocomplete.iniciar();
	
	var tabelaItens = new Drinks.TabelaItens(autocomplete);
	tabelaItens.iniciar();
	
	var venda = new Drinks.Venda(tabelaItens);
	venda.iniciar();
